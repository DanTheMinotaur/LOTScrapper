...
class Parser:
    ...
    def add_lotto_54321(self, single_game=True):
        if 'lotto_54321' in self.game_results:
            data_to_insert = []
            ...
            print("No Lotto54321 Results to Parse")


    def add_daily_millions(self, single_game=True):
        if 'daily-million' in self.game_results:
            data_to_insert = []
            ...
            print("No Daily Millions Results to Parse")

    def add_euromillions(self, single_game=True):
        print(self.game_results)
        if 'euromillions' in self.game_results:
            eur_mil = []
            eur_mil_plus = []
            ...
            print("No Euromillion Results to Parse")

    def add_lotto(self, single_game=True):
        if 'lotto' in self.game_results:
            data_to_insert = []
            ...
            print("No Lotto Results to Parse")
    ...
    def read_games(self, game_date):
        for game in self.games:
            result = self.read_page('download/' + game_date + '/' + game + '.html')
            #print(result)
            if result is not None:
                self.game_results[game] = result
